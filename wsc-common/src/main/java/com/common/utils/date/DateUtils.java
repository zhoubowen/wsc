...
//  }
   ...
   */
	...
	public static final String getTimeDisparity(Date date) {
		...
		if (isNowYear) {
			return DateUtils.getDate(date, "MM月dd日");
		} else {
			return DateUtils.getDate(date, "yyyy年MM月dd日");
		}
  ...
  public static void main(String[] args) {
    String beginTime = DateUtils.getDate(DateUtils.getDate("2016-08-15 12:50:00"), DateUtils.HOUR_MINUTE);
    Date today = DateUtils.getDate(DateUtils.DATE_DAY_FORMAT, new Date()); //今天 add by bowen on 2016-09-22
    today = DateUtils.addDay(today, 1); //明天

    System.out.println(DateUtils.getDateZero(new Date()));
